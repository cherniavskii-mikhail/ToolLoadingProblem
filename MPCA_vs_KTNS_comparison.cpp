...
unsigned int MPCA(TLP_Data& tlp_data) {
    ...
    */
    ...
    for (end = 0; end < tlp_data.numJobs; end++) {
        ...
        if (tlp_data.filled_slots[end] == tlp_data.magCapacity) { last_full = end; }
    }
...
unsigned int MPCA_bitwise(TLP_Data& tlp_data) {
    ...
    */
    ...
    for (int end = 1; end < tlp_data.numJobs; end++) {
        if (tlp_data.job_seqence_free[end - 1] == 0) { fullmag = end - 1; }
        ...
        for (int start = end - 2; start >= 0; start--) {
            ...
            if (min_empty > tlp_data.job_seqence_free[start + 1]) { min_empty = tlp_data.job_seqence_free[start + 1]; }
            ...
            if (intersect > 0) {
                ...
                if (pipes_count > min_empty) { pipes_count = min_empty; }
                if (pipes_count > 0) {
                    for (int j = start + 1; j < end; j++) {
                        tlp_data.job_seqence_free[j] -= pipes_count;
                        if (tlp_data.job_seqence_free[j] == 0) { fullmag = j; }
                    }
